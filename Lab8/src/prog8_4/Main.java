...
public class Main {
	...
	public void printAsUpperCase() {
		...
		listOfAddresses.forEach(a->System.out.println(formatFields(a.toUpper())));
	}
	
	private String formatFields(List<String> list) {
		
		return String.format("%s %n%s %n%s, %s, %s %n", list.get(0),list.get(1),list.get(2),list.get(3),list.get(4));
